...
# import data
rm(list = ls())
train <- read.csv('./data/gold_data_PCA/train.csv')
val <- read.csv('./data/gold_data_PCA/val.csv')
test_X <- read.csv('./data/gold_data_PCA/test.csv')
...
set.seed(1)
bagging.rate <- randomForest(average_daily_rate ~ ., data = train, mtry = 96, importance = TRUE)
...
# save model
save(xgb.tune.rate, file = "models/xgb_model7_train_PCA.Rdata")
